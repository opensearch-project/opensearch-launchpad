...
For full configuration options, authentication methods, and advanced features, see the [opensearch-mcp-server-py User Guide](https://github.com/opensearch-project/opensearch-mcp-server-py/blob/main/USER_GUIDE.md).

If the MCP server is not available, auto-install it before proceeding:

1. Ask the user for their OpenSearch connection details:
   - **Endpoint URL** — e.g., `https://localhost:9200` or `https://my-domain.us-east-1.es.amazonaws.com`
   - **Authentication method**:
     - Local/self-managed: username and password
     - Amazon OpenSearch Service (AOS): AWS profile name or IAM role ARN + region
     - Amazon OpenSearch Serverless (AOSS): AWS profile name + region
   - If the user is unsure, ask: "Is this a local OpenSearch cluster, Amazon OpenSearch Service, or Amazon OpenSearch Serverless?"

2. Locate the MCP config file for the current IDE:
...
3. Read the existing config (or start with `{"mcpServers": {}}` if the file doesn't exist).

4. Add the `opensearch-mcp-server` entry with the user's connection details. Choose the right env block:
   ...
   ```json
   {
     "opensearch-mcp-server": {
       ...
       "env": {
         "OPENSEARCH_URL": "<user_provided_url>",
         "OPENSEARCH_USERNAME": "<user_provided_username>",
         "OPENSEARCH_PASSWORD": "<user_provided_password>",
   ...
   For Amazon OpenSearch Service (AOS) with AWS profile:
   ```json
   {
     "opensearch-mcp-server": {
       "command": "uvx",
   ...
   For Amazon OpenSearch Serverless (AOSS) with AWS profile:
   ```json
   {
     "opensearch-mcp-server": {
       "command": "uvx",
...
5. Save the file. Do not overwrite existing entries.

6. Ask the user to restart their IDE or reconnect MCP servers for the changes to take effect.

7. Once the MCP server is connected, verify it works by calling `ListIndexTool` or `ClusterHealthTool`.
